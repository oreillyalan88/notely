if(Meteor.isServer){
    Meteor.methods({
      ...
      addComments: function(commentInput, postId){
       ...
       joinRequest: function(thisModule, Name){
            if(!Meteor.userId()){
               throw new Meteor.Error('not authorized')
               this.stop();
               return false;
               ...
               Modules.update(thisModule,{$addToSet: {requested: {name: Name}}});
               
           }
       },
              
       approveRequest: function(thisModule, Name){
            if(!Meteor.userId()){
               throw new Meteor.Error('not authorized')
               this.stop();
               return false;
               ...
               Modules.update(thisModule,{$addToSet: {approved: {name: Name}}});
               Modules.update(thisModule,{$pull:     { requested: { $in: [Name] }}});

           }
       },       
       
       rejectRequest: function(thisModule, Name){
            if(!Meteor.userId()){
               throw new Meteor.Error('not authorized')
               this.stop();
               return false;
               ...
               Modules.update(thisModule,{$addToSet: {requested: Name}});
