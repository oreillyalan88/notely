    ...
    Meteor.methods({
       ...
       addPosts: function(postInput, moduleId){
           if(!Meteor.userId()){
               ...
               Meteor.users.update({_id: userId},{$inc: {uploadScore: +countUploads}})
               }

               TempPostCollection.remove({userId: Meteor.userId()})
               if(countUploads == 0 || countUploads ==undefined){
                   countUploads = 0
               }
        ...
        removePost: function(postId){
           if(!Meteor.userId()){
               ...
               var uploadScore= Posts.findOne({_id:postId}).countUploads
               var userId= Posts.findOne({_id:postId}).userId

               if (uploadScore!=undefined || uploadScore>0){

                   var countUploads = uploadScore
                   Meteor.users.update({_id: userId},{$inc: {'uploadScore': -countUploads}})
                   var isNull = Meteor.users.findOne({_id: userId}).uploadScore
                   if(isNaN(isNull)){
                       console.log('you got here')
                       Meteor.users.update({_id: userId},{$set: {'uploadScore': 0}})
                   }
               }
        ...
        deletePostByUpload: function(uploadId,postId){
            if(!Meteor.userId()){
                throw new Meteor.Error('not authorized')
                this.stop();
                return false;
        ...
        updateAllPostImages: function(thisUser, thisImage){
           if(!thisUser){
               ...
               let userName = Meteor.users.findOne({_id:thisUser}).username
      ...
      addComments: function(commentInput, postId, id){
       ...
       deleteRequest:function(adminId, thisModule, Name){
        ...
        deleteApproval:function(adminId, thisModule, Name){
            if(!Meteor.userId()){
                throw new Meteor.Error('not authorized')
                this.stop();
                return false;
                ...
                Module.update({admin_id:adminId, moduleName: thisModule},{ $pull: { approved: { name : Name} } })
            }
        },
       ...
       rejectRequest: function(adminId, thisModule, Name){
            if(!Meteor.userId()){
               ...
               return false;
                ...
                Module.update({admin_id:adminId, moduleName: thisModule},{$addToSet: {rejected: {name: Name}}});
