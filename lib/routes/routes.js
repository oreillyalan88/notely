...
Router.map(function(){
    ...
    this.route('modules', {
     ...
     data: function(){
        var college_slug = this.params.college_slug;
        var department_slug = this.params.department_slug;
        var course_slug = this.params.course_slug;
        var year_slug = this.params.year_slug;

        var course = titleCase(course.replace(/-/g,' ')); 
        var year = titleCase(course.replace(/-/g,' ')); 
        var departmentId = titleCase(course.replace(/-/g,' ')); 
        ...
        viewData = {
          ...
          modules: Modules.find({course: course_slug, year:year, departmentId:departmentId}).fetch()
     }
        return viewData
        window.scrollTo(0, 0);    
